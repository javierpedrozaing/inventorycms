...
function get_agenda_product($flores_id)
{
  global $db;
  $id = (int)$flores_id;
  $sql = $db->query("SELECT * FROM agenda WHERE flores_id='{$db->escape($id)}' LIMIT 1");
  if($result = $db->fetch_assoc($sql))
            return $result;
          else
            return null;
 ...
 function updateLastLogIn($user_id)
  ...
  /*--------------------------------------------------------------*/
   ...
   function find_product_by_title($product_name){
     ...
     $sql = "SELECT id FROM flores WHERE nombre like '%$p_name%' LIMIT 5";
     $result = find_by_sql($sql);
     return $result;
   }
